    ...
    if not df_map.empty and gdf_shape is not None:
        ...
        if selected_kab and selected_kec:
            ...
            if not final_gdf.empty:
                ...
                # --- DEBUGGING KHUSUS UNTUK MELIHAT ERRORNYA APA ---
                st.error(f"Wilayah SHP tidak ditemukan untuk {selected_kec}.")
                st.info(f"Data yang dicari di SHP: Kabupaten='{selected_kab}', Kecamatan='{selected_kec}'")
                            ...
                            try:
                                ...
                                if eb_plot_type == "Line Chart":
                                    fig.add_trace(go.Scatter(
                                        x=df_agg[eb_cat],
                                        y=df_agg['mean'],
                                        error_y=dict(type='data', array=df_agg['error_val'], visible=True),
                                        mode='lines+markers'
                                    ))
                                else: 
