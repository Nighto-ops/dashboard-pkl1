...
if uploaded_file is None:
    ...
    if not df_map.empty and gdf_shape is not None:
        ...
        if selected_kab and selected_kec:
            ...
            # Perbaikan Paksa untuk Gedang Sari dan lainnya
            gdf_shape['kec_upper'] = gdf_shape['kec_upper'].str.replace('Gedang Sari', 'Gedangsari', regex=False)
            gdf_shape['kec_upper'] = gdf_shape['kec_upper'].str.replace('Sapto Sari', 'Saptosari', regex=False)

            # Mapping Tambahan (Jaga-jaga)
...
# === JIKA FILE SUDAH DIUPLOAD (KODE ASLI STATISTIK ANDA 100%) ===
else:
    ...
    if df is not None and all_cols:
            ...
            if len(numeric_cols) < 2:
                ...
                with col1:
                    ...
                    if len(pca_vars) < 2:
                        ...
                        n_components_pca = st.slider("Pilih jumlah Komponen PCA:", min_value=1, max_value=len(pca_vars), value=max(1, len(pca_vars)//2), key='n_pca')
                with col2:
                    if len(pca_vars) >= 2:
                ...
                with col3:
                    ...
                    if len(efa_vars) < 3:
                        ...
                        n_components_efa = st.slider("Pilih jumlah Faktor:", min_value=1, max_value=len(efa_vars)-1, value=max(1, len(efa_vars)//2), key='n_efa')
        ...
        with tab_class:
            ...
            if not numeric_cols:
                ...
                col1, col2 = st.columns([1, 2])
                with col1:
                    cluster_vars = st.multiselect("Pilih variabel numerik untuk Clustering:", numeric_cols, default=numeric_cols if len(numeric_cols) >= 2 else [], key='clust_vars')
                    n_clusters = st.slider("Pilih jumlah Cluster (K):", min_value=2, max_value=10, value=3, key='n_clust')
                with col2:
                    if len(cluster_vars) >= 2:
                        if st.button("Jalankan K-Means Clustering", key='clust_btn'):
                            try:
                                ...
                                fig_clust = px.scatter(df_clean_clust, x=cluster_vars[0], y=cluster_vars[1], color='Cluster', title=f"Hasil Clustering K-Means (K={n_clusters})", color_continuous_scale=px.colors.qualitative.Plotly)
                ...
                with col3:
                    if not categorical_cols:
                        ...
                        lda_predictors = st.multiselect("Pilih Variabel Prediktor (Numerik):", available_lda_x, default=available_lda_x if len(available_lda_x) >= 2 else [], key='lda_preds')
                with col4:
